...
module.exports = {
    ...
    basicStructures : {
        ...
        testDescribesBasicStructures: function(test) {
            ...
            test.ok(description.indexOf(`sep ${entryID} ${E("title")} ${E("Website")}`) >= 0, "Entry should have title");
            test.ok(description.indexOf(`sep ${entryID} ${E("username")} ${E("name")}`) >= 0, "Entry should have username");
            test.ok(description.indexOf(`sep ${entryID} ${E("password")} ${E("code")}`) >= 0, "Entry should have password");
            test.ok(description.indexOf(`sem ${entryID} ${E("metaItem1")} ${E("123 456")}`) >= 0, "Entry should have meta");
            test.ok(description.indexOf(`sea ${entryID} ${E("my attribute")} ${E("attr value")}`) >= 0, "Entry should have attribute");
            ...
            test.ok(description.indexOf(`sga ${groupID} ${E("testAttr")} ${E("groupAttrValue")}`) >= 0, "Group should have an attribute");
